...
def merge(db_files=None, out_file=None, overwrite=True, attempt_salvage=True):
    ...
    if not out_file:
        out_file = 'correlated/sms_act.db'
    ...
    out_cursor.execute('create table %s (id int, device text, date int, type_of_act int, type_of_sms int)' % data_table)
    ...
    for db_file in db_files:
        ...
        try: 
            ...
            for row in cursor1:
                ...
                cursor2.execute("select * from %s where probe = 'sms' and date <= %d and date >= %d and datatype = 2" % (data_table, date+tolerance, date-tolerance))
